...
(function (factory) {
    ...
    $.timeago = function(timestamp) {
        if (timestamp instanceof Date) {
            ...
            return getTimeDifference($.timeago.parse(timestamp));
        } else if (typeof timestamp === "number") {
            return getTimeDifference(new Date(timestamp));
        } else {
            return getTimeDifference($.timeago.datetime(timestamp));
        }
    };
    var $t = $.timeago;
    ...
    function refresh() {
        ...
        if (!isNaN(data.datetime)) {
            if ( $s.cutoff === 0 || Math.abs(distance(data.datetime)) < $s.cutoff) {
                $(this).text(getTimeDifference(data.datetime));
            } else {
    ...
    function prepareData(element) {
        ...
        if (!element.data("timeago")) {
            ...
            if ($t.settings.localeTitle) {
                localday = convertToLocalTime(element.data('timeago').datetime);
                element.attr("title", convertToPrettyTime(localday));
    ...
    function getTimeDifference(date) {
        localDate = convertToLocalTime(date);
